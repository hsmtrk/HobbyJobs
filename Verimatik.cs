...
namespace Verimatik
{
    ...
    public  class VM
    {
       ...
       /// <returns>Json sonucunu string değişkeni olarak döndürür</returns>
        ...
        /// <summary>
        /// ProcedureCalistir fonksiyonu adı belirtilen stored procedure nesnesini çalıştırır.
        /// Ancak geriye bir değer döndürmez!
        /// </summary>
        /// <param name="ProsedurAdi">İlgili Stored Procedure adı</param>
        /// <param name="ParametreDegerleri">Eğer varsa Parametre değerleri</param>
        public static void ProcedureCalistir(string ProsedurAdi, ArrayList ParametreDegerleri)
        {
            try
            {
                ProsedurAdi = ProsedurAdi.Trim();// From İfadesinden sonra gelecek ifade güvenli olmalı ! Bu kontrol ediliyor...
                ProsedurAdi = ProsedurAdi.Replace("'", "");

                SqlCommand cmd = new SqlCommand();
                DataTable dt = Listele("select PARAMETER_NAME  from INFORMATION_SCHEMA.PARAMETERS where SPECIFIC_NAME ='" + ProsedurAdi + "'");
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = ProsedurAdi;

                if (ParametreDegerleri.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        cmd.Parameters.AddWithValue(dt.Rows[i][0].ToString(), ParametreDegerleri[i].ToString());
                    }
                }
        ...
        /// <summary>
        /// ProcedureCalistir_TabloGetir fonksiyonu adı belirtilen stored procedure nesnesini çalıştırır.
        /// </summary>
        /// <param name="ProsedurAdi">İlgili Stored Procedure adı</param>
        /// <param name="ParametreDegerleri">Eğer varsa Parametre değerleri</param>
        /// <returns>Stored Procedure nesnesininden dönen değer kümesini DataTable nesnesi olarak verir.</returns>
        public static DataTable ProcedureCalistir_TabloGetir(string ProsedurAdi, ArrayList Parametreler)
        {
            try
            {
                ProsedurAdi = ProsedurAdi.Trim();// From İfadesinden sonra gelecek ifade güvenli olmalı ! Bu kontrol ediliyor...
                ...
                if (Parametreler.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        cmd.Parameters.AddWithValue(dt.Rows[i][0].ToString(), Parametreler[i].ToString());
                    }
                }
        ...
        /// <summary>
        /// Listele_HTMLTablo fonksiyonu, adı belirtilen tablonun kayıtlarını HTML Tablosu olarak oluşturur
        /// </summary>
        /// <param name="TabloAdi">İlgili Tablo adı</param>
        /// <param name="Ozellikler">Html tablo tagına eklenebilecek clas, style vb. özellikler (Attribute :) )</param>
        /// <returns>Tablodaki kayıtlar string formatında HTML Tablo olarak geri gönderilir.</returns>
        public static string Listele_HTMLTablo(string SqlCumlecigi, string Ozellikler)
        {

            string HTML_Tablo = "<table>\n";
            if (Ozellikler != "")
            {
                HTML_Tablo = "<table " + Ozellikler + ">\n";
            }

            if (SqlCumlecigi != "")
            {
                #region Başlık satırı oluşturuluyor...

                HTML_Tablo += "<tr>\n";//Başlık kolonları için bir satır ekliyoruz...
                DataTable dt = Listele(SqlCumlecigi);
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    HTML_Tablo += "<th>" + dt.Columns[i].ToString() + "</th>\n";
                }
                HTML_Tablo += "</tr>\n";//Başlık kolonları için eklediğimiz satırı kapatıyoruz...

                #endregion

                #region Kayıt satırları oluşturuluyor...
                ...
                for (int k = 0; k < dt.Rows.Count; k++)
                {
                    ...
                    for (int j = 0; j < dt.Columns.Count; j++)
                    {
                        HTML_Tablo += "<td>" + dt.Rows[k][j] + "</td>\n";
                    }
                    HTML_Tablo += "</tr>\n";// satırı kapanışı...
                }
                #endregion

                HTML_Tablo += "</table>"; // Tablo kapanışı...
        ...
        /// <summary>
        /// Listele_HTMLTablo fonksiyonunun bu versiyonu, DataTable nesnesini, HTML Tablosuna dönüştürür.
        /// </summary>
        /// <param name="Tablo">DataTable nesnesi</param>
        /// <param name="Ozellikler">Html tablo tagına eklenebilecek clas, style vb. özellikler (Attribute :) )</param>
        /// <returns>Tablodaki kayıtlar string formatında HTML Tablo olarak geri gönderilir.</returns>
        public static string Listele_HTMLTablo(DataTable Tablo, string Ozellikler)
        {
